...
public class PaymentMessage implements Serializable{
    ...
    public static PaymentMessage[] createPayMentMsg(OrderMessage orderMessage) {
        ...
        for (short i = 0; i < 2; i++) {
            ...
            msg.createTime = orderMessage.getCreateTime() + rand.nextInt(100);//支付时间模拟为创建订单时间戳增加0-99 jason
            msg.payAmount = 0.0;
            list[i] = msg;
        }

        list[0].payAmount = rand.nextInt((int) (orderMessage.getTotalPrice() / 2));//某次支付金额模拟为0到二分之一总价 jason
        list[1].payAmount = orderMessage.getTotalPrice() - list[0].payAmount; //剩下的则为总额减去前者 jason
